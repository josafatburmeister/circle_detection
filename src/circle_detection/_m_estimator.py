...
class MEstimator(CircleDetector):  # pylint: disable=too-many-instance-attributes
    ...
    For each start position, :code:`n_start_radius` different starting values for the circle radius
    :math:`r` are tested, evenly covering the interval between :code:`min_start_radius ` and :code:`max_start_radius`.
