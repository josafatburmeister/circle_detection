...
class MEstimator(CircleDetector):  # pylint: disable=too-many-instance-attributes
    ...
    A grid of starting values with a cell size of :code:`(max_start_x - min_start_x) / (n_start_x + 1)` x
    :code:`(max_start_y - min_start_y) / (n_start_y + 1)` is generated and the points on the grid border are not used
    as starting values. For each start position, :code:`n_start_radius` different starting values for the circle radius
    :math:`r` are tested, evenly covering the interval between
    :code:`min_start_radius + (max_start_radius - min_start_radius) / (n_start_radius + 1)` and
    :code:`max_start_radius - (max_start_radius - min_start_radius) / (n_start_radius + 1)`.
